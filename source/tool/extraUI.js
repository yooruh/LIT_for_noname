...
const DialogManager = (() => {
    ...
    const _setupClickOutsideToClose = (overlay, cancelCallback) => {
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay && cancelCallback) {
                const cancelBtn = overlay.querySelector('button[data-cancel="true"]');
                cancelBtn?.click();
            }
        });
    };
        ...
        if (enableBackClose) {
            ...
            const handlePopState = (event) => {
                if (overlay.parentNode === document.body) {
                    ...
                    if (originalHistoryState && originalHistoryState.dialogOpen) {
    ...
    return {
        ...
        async fileManager(title, message, items) {
            ...
            return new Promise((resolve) => {
                ...
                const deleteBtn = _createButton('删除', {
                    ...
                    onClick: () => {
                        if (overlay.parentNode) document.body.removeChild(overlay);
                        resolve({ action: 'delete', files: Array.from(selectedFiles) });
                    }
                });
                ...
                const editBtn = _createButton('编辑', {
                    ...
                    onClick: () => {
                        if (overlay.parentNode) document.body.removeChild(overlay);
                        resolve({ action: 'edit', files: Array.from(selectedFiles) });
                    }
                });
                ...
                const applyBtn = _createButton('应用配置', {
                    ...
                    onClick: () => {
                        if (overlay.parentNode) document.body.removeChild(overlay);
                        resolve({ action: 'apply', files: Array.from(selectedFiles) });
                    }
                });
        ...
        async showCountdownDialog(title, message, options = {}) {
            ...
            return new Promise((resolve) => {
                ...
                overlay.appendChild(dialog);
                document.body.appendChild(overlay);
                _setupClickOutsideToClose(overlay, onCancel);
        ...
        async showDocModal(url, title, dataProcessor = null) {
            ...
            return new Promise((resolve) => {
                ...
                overlay._cleanup = _addDialogEventHandlers(overlay, closeModal, {
                    enableEscClose: true,
                    enableBackClose: true,
                    enableOverlayClickClose: true
                });
